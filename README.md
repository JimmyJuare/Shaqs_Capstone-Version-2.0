# Shaqs Capstone – Version 2.0

## Key Features / Enhancements in Version 2.0
- Power BI enhancements (slicers, new charts)
- New visuals / DAX measures / pages
- Performance improvements, etc.

## Live Dashboard
https://app.powerbi.com/links/Ll1xeGrqDI?ctid=bbce5c37-f181-4d0c-9310-7f877336e1cf&pbi_source=linkShare



**Authors:** Onur Karaer, Tracy Arista, Shaquan Bowens, Maritza Ruiz, Teodoro Jimmy Juarez 

**Territory:** Maryland – Northeast Region 

**Tools:** SQL, Excel PivotCharts, Power Point, Power BI



**Project Overview**

This project analyzes EmporiUm’s Maryland sales territory within the Northeast Region. Version 2.0 enhances the original SQL + Excel work by adding a fully interactive Power BI dashboard, enabling deeper insights and stronger data storytelling.



**Business Scenario**

EmporiUm sells textbooks, technology, art supplies, and merchandise both in-store and online. Sales Manager Shruti Reddy requested a complete Maryland territory report including revenue trends, store rankings, category performance, and recommendations for the next quarter.



**Summary of Version 1 (SQL + Excel)**

•	Maryland shows strong seasonal peaks, especially in November.

•	IT products dominate revenue.

•	Store performance is uneven, with one store heavily outperforming the rest.

•	Online sales lag behind in-store sales.



**Initial Recommendation:** Grow online sales during peak months and maintain strong IT inventory.



**Power BI Enhancements (Version 2.0)**

The new dashboard reveals:

•	Maryland contributes 39% of Northeast revenue.

•	KPIs: 92K transactions, $140 AOV, 9 stores.

•	Online sales underperform across all categories.

•	North Harford generates $8.7M, far above other stores.

•	Tech products lead regional performance.

•	Maryland’s national share: 11% online vs. 89% in-store.

Power BI slicers make it easy to compare in-store vs. online performance, exposing gaps and opportunities not visible in Excel.



**Final Recommendations (Version 2.0)**

1\.	Launch a Maryland online push focused on tech, where digital demand already exists.

2\.	Diversify beyond North Harford by piloting its successful model in 2–3 additional stores.

3\.	Use Power BI continuously to monitor category and channel performance.



**Repository Contents**

•	shaqs_capstone_1.odp – Shaq's Original Capstone


•	Shaqs_Capstone_Version_2.0.pbix – Final enhanced Power BI Dashboard

•	Published Dashboard: https://app.powerbi.com/links/Ll1xeGrqDI?ctid=bbce5c37-f181-4d0c-9310-7f877336e1cf&pbi_source=linkShare

•	README.md – Project Documentation



